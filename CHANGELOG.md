...
- **`reset_function_prototype` guarded signature reset (1 new endpoint).** Returns
  ...
  guard defaults to `USER_DEFINED`; resetting `ANALYSIS`, `IMPORTED`, or already
  `DEFAULT` state requires naming that source explicitly. Thunks require the
  narrowly scoped `allow_thunk=true` override. Address resolution, guard check,
  reset, structured before/after snapshot, metadata-preservation check, and
  verification run in one transaction and roll back together on failure. Tool
  count 274 → 275.

- **`search_raw_strings` exact-byte fallback (1 new read-only endpoint).** Searches
  literal ASCII, UTF-8, and/or UTF-16LE bytes with Ghidra's memory search across
  loaded, initialized blocks, including literals that begin inside a larger
  defined string. Results include address-space, block, encoding, byte length,
  adjacent-byte boundaries, and containing-defined-data offsets, with explicit
  evidence language that a hit proves byte occurrence only—not a string boundary
  or reference. Pagination, query size, and scanned results are bounded; identical
  ASCII/UTF-8 hits are deduplicated. Existing `/search_strings` regex/defined-data
  behavior is unchanged. Tool count 273 → 274.

- **`project_file_status` lifecycle preflight (1 new endpoint).** GUI and
  headless project modes can now inspect a real `DomainFile` before a shared
  repository operation: local/latest versions, checkout owner/id/version,
  dirty and busy state, `can_checkout`/`can_checkin`/`can_merge`, explicit
  blocker lists, and the next safe action. GUI
  `/server/repository/file` returns the same enriched project-file snapshot and
  checkout/check-in responses include post-operation status. Headless
  repository-adapter browsing explicitly reports local project-file state as
  `not_supported` instead of implying it can see checkout or merge readiness.
  No automatic merge endpoint was added; callers must resolve
  `merge_required` deliberately. Tool count 272 → 273.
