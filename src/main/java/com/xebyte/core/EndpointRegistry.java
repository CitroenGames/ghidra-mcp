...
public class EndpointRegistry {
    ...
    private void registerListingEndpoints() {
        ...
        get("/search_raw_strings", "Search exact literal bytes in loaded, initialized memory",
            params(qStr("search_term", "Required literal text (not a regex)"),
                qStr("encoding", "ascii, utf8, utf16le, or all"), qInt("offset", 0),
                qInt("limit", 100), pProg()),
            (q, b) -> listingService.searchRawStrings(str(q, "search_term"), str(q, "encoding"),
    ...
    private void registerFunctionEndpoints() {
        ...
        post("/reset_function_prototype", "Reset stored prototype to Ghidra defaults with a signature-source guard",
            params(bStr("function_address"),
                bStrOpt("expected_signature_source", "USER_DEFINED"),
                bBool("allow_thunk", false), pProg()),
            (q, b) -> functionService.resetFunctionPrototype(
                bodyStr(b, "function_address"),
                bodyStr(b, "expected_signature_source", "USER_DEFINED"),
                bodyBool(b, "allow_thunk", false), str(q, "program")));
