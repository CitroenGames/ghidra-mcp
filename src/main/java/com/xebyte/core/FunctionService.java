...
public class FunctionService {
    ...
    /**
     * Remove a stored function signature without inventing a replacement prototype.
     *
     * <p>This is deliberately guarded by the live signature source.  The default guard is
     * {@link SourceType#USER_DEFINED}, which makes the common "undo my forced prototype" case
     * concise while requiring callers to name {@code ANALYSIS}, {@code IMPORTED}, or
     * {@code DEFAULT} explicitly.  Resolution, compare-and-set, mutation, and verification all
     * happen in one {@link ThreadingStrategy#executeWrite} transaction so a concurrent signature
     * change cannot slip between the guard and the write.
     */
    @McpTool(path = "/reset_function_prototype", method = "POST",
        description = "Reset a function's stored prototype to Ghidra's default/inferred state without supplying a guessed signature. Compare-and-set guarded by expected_signature_source (default USER_DEFINED); ANALYSIS, IMPORTED, and DEFAULT must be named explicitly. Thunks are refused unless allow_thunk=true. Returns structured before/after state and rolls back on verification failure.",
        category = "function")
    public Response resetFunctionPrototype(
            @Param(value = "function_address", paramType = "address", source = ParamSource.BODY,
                   description = "Function address. Accepts 0x<hex> or <space>:<hex>.") String functionAddress,
            @Param(value = "expected_signature_source", source = ParamSource.BODY,
                   defaultValue = "USER_DEFINED",
                   description = "Compare-and-set guard: USER_DEFINED (default), ANALYSIS, IMPORTED, or DEFAULT.") String expectedSignatureSource,
            @Param(value = "allow_thunk", source = ParamSource.BODY, defaultValue = "false",
                   description = "Explicitly allow resetting the effective signature reached through a thunk.") boolean allowThunk,
            @Param(value = "program", description = "Target program name", defaultValue = "") String programName) {
        ...
        try {
            expectedSource = parseExpectedSignatureSource(expectedSignatureSource);
        }
        ...
        try {
            return threadingStrategy.executeWrite(program, "Reset function prototype", () -> {
                Address address = ServiceUtils.parseAddress(program, functionAddress);
                if (address == null) {
                    throw new SignatureResetException(ServiceUtils.getLastParseError());
                }
                Function function = ServiceUtils.getFunctionForAddress(program, address);
                if (function == null) {
                    throw new SignatureResetException(
                        "No function found at or containing address " + functionAddress);
                }

                SourceType liveSource = function.getSignatureSource();
                if (liveSource != expectedSource) {
                    throw new SignatureResetException(
                        "signature_source guard mismatch at " + functionAddress
                            + ": expected " + expectedSource.name()
                            + " but live source is " + liveSource.name()
                            + ". Re-read the function and pass the live source explicitly only if "
                            + "that signature is the intended reset target.");
                }
                if (function.isThunk() && !allowThunk) {
                    throw new SignatureResetException(
                        "Refusing to reset thunk " + function.getName()
                            + " without explicit allow_thunk=true; Ghidra signature writes on a "
                            + "thunk affect its effective thunk target.");
                }

                Map<String, Object> before = snapshotFunctionPrototypeState(function);
                if (liveSource == SourceType.DEFAULT) {
                    Map<String, Object> result = new LinkedHashMap<>();
                    result.put("status", "noop");
                    result.put("function_address", function.getEntryPoint().toString());
                    result.put("expected_signature_source", expectedSource.name());
                    result.put("allow_thunk", allowThunk);
                    result.put("idempotent", true);
                    result.put("verified", true);
                    result.put("before", before);
                    result.put("after", before);
                    return Response.ok(result);
                }

                resetFunctionPrototypeState(function, program);
                Map<String, Object> after = snapshotFunctionPrototypeState(function);
                verifyFunctionPrototypeReset(before, after, function);
            ...
            return Response.err("Failed to reset function prototype: " + message);
        }
    }

    static SourceType parseExpectedSignatureSource(String value) {
        String normalized = value == null || value.isBlank()
            ? SourceType.USER_DEFINED.name()
            : value.trim().toUpperCase(Locale.ROOT).replace('-', '_');
        SourceType source;
        try {
            source = SourceType.valueOf(normalized);
        }
        catch (IllegalArgumentException e) {
            throw new IllegalArgumentException(
                "expected_signature_source must be one of USER_DEFINED, ANALYSIS, "
                    + "IMPORTED, or DEFAULT; got '" + value + "'");
        }
        if (source != SourceType.USER_DEFINED && source != SourceType.ANALYSIS
                && source != SourceType.IMPORTED && source != SourceType.DEFAULT) {
            throw new IllegalArgumentException(
                "expected_signature_source " + source.name()
                    + " is not resettable; use USER_DEFINED, ANALYSIS, IMPORTED, or DEFAULT");
        }
        return source;
    }

    /** Apply only signature-state changes; the surrounding executeWrite owns rollback. */
    void resetFunctionPrototypeState(Function function, Program program) throws Exception {
        resetFunctionPrototypeState(
            function, createDefaultReturnParameter(program), DataType.DEFAULT);
    }

    Variable createDefaultReturnParameter(Program program) throws Exception {
        return new ReturnParameterImpl(DataType.DEFAULT, program);
    }

    static void resetFunctionPrototypeState(Function function, Variable defaultReturn,
            DataType defaultReturnType)
            throws Exception {
        function.updateFunction(
            ...
            false,
            SourceType.DEFAULT);
        function.setReturnType(defaultReturnType, SourceType.DEFAULT);
        function.setVarArgs(false);
        function.setCustomVariableStorage(false);
        function.setSignatureSource(SourceType.DEFAULT);
        // Function.UNKNOWN_STACK_DEPTH_CHANGE is Ghidra's public sentinel for an unknown purge.
        function.setStackPurgeSize(Function.UNKNOWN_STACK_DEPTH_CHANGE);
    }

    Map<String, Object> snapshotFunctionPrototypeState(Function function) {
        Map<String, Object> signature = new LinkedHashMap<>();
        signature.put("source", function.getSignatureSource().name());
        signature.put("calling_convention", normalizedCallingConvention(function));
        signature.put("parameter_count", function.getParameterCount());
        signature.put("explicit_parameter_count", explicitParameterCount(function));
        DataType returnType = function.getReturnType();
        signature.put("return_type", returnType != null ? returnType.getPathName() : null);
        signature.put("return_is_default", isDefaultReturnType(returnType));
        signature.put("custom_variable_storage", function.hasCustomVariableStorage());
        signature.put("varargs", function.hasVarArgs());
        signature.put("stack_purge_size", function.getStackPurgeSize());
        signature.put("stack_purge_unknown",
            function.getStackPurgeSize() == Function.UNKNOWN_STACK_DEPTH_CHANGE);

        Map<String, Object> metadata = new LinkedHashMap<>();
        metadata.put("name", function.getName());
        Namespace parentNamespace = function.getParentNamespace();
        metadata.put("namespace", namespacePath(parentNamespace));
        metadata.put("namespace_id", parentNamespace != null ? parentNamespace.getID() : null);
        metadata.put("comment", function.getComment());
        metadata.put("repeatable_comment", function.getRepeatableComment());
        metadata.put("tags", functionTagNames(function));
        metadata.put("inline", function.isInline());
        metadata.put("no_return", function.hasNoReturn());
        AddressSetView body = function.getBody();
        metadata.put("body_min", body != null && !body.isEmpty()
            ? body.getMinAddress().toString() : null);
        metadata.put("body_max", body != null && !body.isEmpty()
            ? body.getMaxAddress().toString() : null);
        metadata.put("body_ranges", body != null ? body.getNumAddressRanges() : 0);
        metadata.put("body_addresses", body != null ? body.getNumAddresses() : 0L);
        metadata.put("is_thunk", function.isThunk());
        Function thunkTarget = function.isThunk() ? function.getThunkedFunction(false) : null;
        metadata.put("thunk_target", thunkTarget != null
            ? thunkTarget.getEntryPoint().toString() : null);
        metadata.put("external", function.isExternal());

        Map<String, Object> snapshot = new LinkedHashMap<>();
        snapshot.put("signature", signature);
        snapshot.put("preserved_metadata", metadata);
        return snapshot;
    }

    boolean isDefaultReturnType(DataType returnType) {
        return returnType != null && DataType.DEFAULT.isEquivalent(returnType);
    }

    @SuppressWarnings("unchecked")
    static void verifyFunctionPrototypeReset(Map<String, Object> before,
            Map<String, Object> after, Function function) throws SignatureResetException {
        List<String> failures = new ArrayList<>();
        Map<String, Object> signature = (Map<String, Object>) after.get("signature");
        if (!SourceType.DEFAULT.name().equals(signature.get("source"))) {
            failures.add("signature source is " + signature.get("source"));
        }
        if (!Boolean.TRUE.equals(signature.get("return_is_default"))) {
            failures.add("return type is not DataType.DEFAULT");
        }
        if (((Number) signature.get("explicit_parameter_count")).intValue() != 0) {
            failures.add("explicit parameters remain: "
                + signature.get("explicit_parameter_count"));
        }
        String convention = (String) signature.get("calling_convention");
        if (!Function.UNKNOWN_CALLING_CONVENTION_STRING.equals(convention)
            ...
            failures.add("calling convention is " + convention);
        }
        if (Boolean.TRUE.equals(signature.get("custom_variable_storage"))) {
            failures.add("custom variable storage remains enabled");
        }
        if (Boolean.TRUE.equals(signature.get("varargs"))) {
            failures.add("varargs remains enabled");
        }
        if (!Boolean.TRUE.equals(signature.get("stack_purge_unknown"))) {
            failures.add("stack purge is not Ghidra's unknown sentinel");
        }
        if (!Objects.equals(before.get("preserved_metadata"),
                after.get("preserved_metadata"))) {
            failures.add("non-signature metadata changed");
        }
        if (!failures.isEmpty()) {
            throw new SignatureResetException(
                "reset verification failed for " + function.getName() + ": "
                    + String.join("; ", failures));
        }
    }

    private static int explicitParameterCount(Function function) {
        int count = 0;
    ...
    private static String normalizedCallingConvention(Function function) {
        ...
        return convention == null || convention.isBlank()
            ? Function.UNKNOWN_CALLING_CONVENTION_STRING : convention;
    }

    private static String namespacePath(Namespace namespace) {
        return namespace != null ? namespace.getName(true) : null;
    }

    private static List<String> functionTagNames(Function function) {
    ...
    static final class SignatureResetException extends Exception {
        SignatureResetException(String message) {
            super(message);
        }
    }
