...
public class ListingService {
    ...
    static final int MAX_RAW_SEARCH_TERM_CHARS = 1024;
    static final int MAX_RAW_PATTERN_BYTES = 4096;
    static final int MAX_RAW_SEARCH_OFFSET = 100_000;
    static final int MAX_RAW_SEARCH_LIMIT = 1_000;
    static final int MAX_RAW_MATCHES_SCANNED = 100_000;
    ...
    /**
     * Search literal encoded bytes rather than Ghidra's defined-string objects.
     *
     * <p>This is intentionally a separate endpoint from {@code /search_strings}: that
     * endpoint's regex and defined-data semantics are compatibility-sensitive. A raw hit
     * proves only that the bytes occur in loaded, initialized memory. It does not prove a
     * string boundary, string data type, or reference to the bytes.</p>
     */
    @McpTool(path = "/search_raw_strings",
            description = "Search exact literal bytes in loaded, initialized memory. A hit proves byte occurrence, not a string boundary or reference.",
            category = "search")
    public Response searchRawStrings(
            @Param(value = "search_term", description = "Required literal text (not a regex)") String searchTerm,
            @Param(value = "encoding", description = "ascii, utf8, utf16le, or all", defaultValue = "all") String encoding,
            @Param(value = "offset", defaultValue = "0") int offset,
            @Param(value = "limit", defaultValue = "100") int limit,
            @Param(value = "program", description = "Target program name (omit to use the active program — always specify when multiple programs are open)", defaultValue = "") String programName) {
        ServiceUtils.ProgramOrError pe = ServiceUtils.getProgramOrError(programProvider, programName);
        ...
        if (searchTerm == null || searchTerm.isEmpty()) {
            return Response.err("search_term parameter is required and must be nonempty");
        }
        if (searchTerm.length() > MAX_RAW_SEARCH_TERM_CHARS) {
            return Response.err("search_term exceeds the maximum of " + MAX_RAW_SEARCH_TERM_CHARS + " characters");
        }
        String normalizedEncoding = encoding == null || encoding.isBlank()
                ? "all" : encoding.toLowerCase(Locale.ROOT);
        if (!Set.of("ascii", "utf8", "utf16le", "all").contains(normalizedEncoding)) {
            return Response.err("encoding must be one of: ascii, utf8, utf16le, all");
        }
        if (offset < 0 || offset > MAX_RAW_SEARCH_OFFSET) {
            return Response.err("offset must be between 0 and " + MAX_RAW_SEARCH_OFFSET);
        }
        if (limit < 1 || limit > MAX_RAW_SEARCH_LIMIT) {
            return Response.err("limit must be between 1 and " + MAX_RAW_SEARCH_LIMIT);
        }
        if ((long) offset + limit > MAX_RAW_MATCHES_SCANNED) {
            return Response.err("offset + limit must not exceed the raw-search scan cap of "
                    + MAX_RAW_MATCHES_SCANNED);
        }

        List<RawEncodingPlan> plans;
        try {
            plans = rawEncodingPlans(searchTerm, normalizedEncoding);
        ...
        Map<String, RawMatch> uniqueMatches = new LinkedHashMap<>();
        boolean scanTruncated = false;

        outer:
        for (MemoryBlock block : memory.getBlocks()) {
            if (!block.isLoaded() || !block.isInitialized()) continue;

            for (RawEncodingPlan plan : plans) {
                Address cursor = block.getStart();
                while (cursor != null && cursor.compareTo(block.getEnd()) <= 0) {
                    Address hit = memory.findBytes(cursor, block.getEnd(), plan.bytes(), null,
                            true, TaskMonitor.DUMMY);
                    if (hit == null) break;

                    String key = hit.getAddressSpace().getName() + ":" + hit.getOffset()
                            + ":" + plan.hexBytes();
                    RawMatch existing = uniqueMatches.get(key);
                    if (existing == null) {
                        RawMatch match = new RawMatch(block, hit, plan,
                                new ArrayList<>(List.of(plan.encoding())));
                        uniqueMatches.put(key, match);
                        allMatches.add(match);
                        if (allMatches.size() >= MAX_RAW_MATCHES_SCANNED) {
                            scanTruncated = true;
                            break outer;
                        }
                    } else {
                        if (!existing.equivalentEncodings().contains(plan.encoding())) {
                            existing.equivalentEncodings().add(plan.encoding());
                        }
                    }
        ...
        int from = Math.min(offset, total);
        int to = Math.min(from + limit, total);
        List<Map<String, Object>> page = allMatches.subList(from, to).stream()
                .map(match -> rawMatchToJson(program, memory, match))
                .toList();
        return Response.ok(JsonHelper.mapOf(
                "matches", page,
                "total_scanned", total,
                "total_is_exact", !scanTruncated,
                ...
                "scope", "loaded_initialized_memory",
                "evidence_note", "A hit proves exact byte occurrence only; it does not prove a string boundary, data type, or reference."
        ));
    }

    private record RawEncodingPlan(String encoding, byte[] bytes, String hexBytes) {}

    private record RawMatch(MemoryBlock block, Address address, RawEncodingPlan plan,
                            List<String> equivalentEncodings) {}

    private static List<RawEncodingPlan> rawEncodingPlans(String term, String encoding) {
        validateUnicodeScalarSequence(term);
        List<RawEncodingPlan> plans = new ArrayList<>();
        if (encoding.equals("ascii") || encoding.equals("all")) {
            boolean ascii = term.chars().allMatch(c -> c <= 0x7f);
            if (!ascii && encoding.equals("ascii")) {
                throw new IllegalArgumentException("search_term contains non-ASCII characters");
            }
            if (ascii) plans.add(rawPlan("ascii", term.getBytes(StandardCharsets.US_ASCII)));
        }
        if (encoding.equals("utf8") || encoding.equals("all")) {
            plans.add(rawPlan("utf8", term.getBytes(StandardCharsets.UTF_8)));
        }
        if (encoding.equals("utf16le") || encoding.equals("all")) {
            plans.add(rawPlan("utf16le", term.getBytes(StandardCharsets.UTF_16LE)));
        }
        return plans;
    }

    private static void validateUnicodeScalarSequence(String term) {
        for (int i = 0; i < term.length(); i++) {
            char value = term.charAt(i);
            if (Character.isHighSurrogate(value)) {
                if (i + 1 >= term.length() || !Character.isLowSurrogate(term.charAt(i + 1))) {
                    throw new IllegalArgumentException("search_term contains an unpaired UTF-16 surrogate");
                }
                i++;
    ...
    private static RawEncodingPlan rawPlan(String encoding, byte[] bytes) {
        if (bytes.length == 0) {
            throw new IllegalArgumentException("search_term encodes to an empty byte sequence");
        }
        if (bytes.length > MAX_RAW_PATTERN_BYTES) {
            throw new IllegalArgumentException(encoding + " search pattern exceeds the maximum of "
                    + MAX_RAW_PATTERN_BYTES + " bytes");
        }
        return new RawEncodingPlan(encoding, bytes, hexBytes(bytes));
    }

    private static Map<String, Object> rawMatchToJson(Program program, Memory memory,
                                                       RawMatch match) {
        MemoryBlock block = match.block();
        Address hit = match.address();
        RawEncodingPlan plan = match.plan();
        Map<String, Object> item = new LinkedHashMap<>();
        item.put("address", hit.toString(false));
        item.put("address_full", hit.toString());
        item.put("address_space", hit.getAddressSpace().getName());
        item.put("block", block.getName());
        item.put("encoding", plan.encoding());
        item.put("equivalent_encodings", List.copyOf(match.equivalentEncodings()));
        item.put("byte_length", plan.bytes().length);
        item.put("matched_bytes_hex", plan.hexBytes());

        Address matchEnd;
        try {
            matchEnd = hit.addNoWrap(plan.bytes().length - 1L);
        } catch (Exception e) {
            matchEnd = hit;
        }
        Map<String, Object> boundary = new LinkedHashMap<>();
        boundary.put("at_block_start", hit.equals(block.getStart()));
        boundary.put("at_block_end", matchEnd.equals(block.getEnd()));
        Byte previous = adjacentByte(memory, block, hit.previous());
        Byte next = adjacentByte(memory, block, matchEnd.next());
        boundary.put("previous_byte_hex", byteHex(previous));
        boundary.put("next_byte_hex", byteHex(next));
        boundary.put("previous_is_nul", previous != null && previous == 0);
        boundary.put("next_is_nul", next != null && next == 0);
        item.put("boundary", boundary);

        Data defined = program.getListing().getDefinedDataContaining(hit);
        if (defined != null) {
            long dataOffset = hit.subtract(defined.getAddress());
            Map<String, Object> definedData = new LinkedHashMap<>();
            definedData.put("address", defined.getAddress().toString(false));
            definedData.put("address_full", defined.getAddress().toString());
            definedData.put("byte_offset", dataOffset);
            definedData.put("length", defined.getLength());
            definedData.put("match_at_start", dataOffset == 0);
            definedData.put("match_fits", dataOffset >= 0
                    && dataOffset + plan.bytes().length <= defined.getLength());
            item.put("containing_defined_data", definedData);
        } else {
            item.put("containing_defined_data", null);
        }
        return item;
    }

    private static Byte adjacentByte(Memory memory, MemoryBlock block, Address address) {
        if (address == null || !block.contains(address)) return null;
        try {
    ...
    private static String byteHex(Byte value) {
        return value == null ? null : String.format("%02x", value & 0xff);
    }
