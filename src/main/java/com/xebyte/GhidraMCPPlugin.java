...
public class GhidraMCPPlugin extends Plugin implements ApplicationLevelPlugin {
    ...
    private void startServer() throws IOException {
        ...
        AnnotationScanner scanner = new AnnotationScanner(programProvider, this,
        ...
        // ==========================================================================
        // PROJECT VERSION CONTROL ENDPOINTS (17 endpoints)
 ...
 */
    ...
    private String getProjectFileInfoJson(String filePath) {
        return getProjectFileLifecycleStatusJson(filePath);
    }
    ...
    private Response projectFileStatus(
            @Param(value = "path", source = ParamSource.QUERY, defaultValue = "",
                description = "Project path. Omit to inspect the current program's DomainFile.")
            String filePath) {
        return Response.text(getProjectFileLifecycleStatusJson(filePath));
    }

    private String getProjectFileLifecycleStatusJson(String filePath) {
        Project project = tool.getProject();
        if (project == null) return "{\"error\": \"No project open\"}";
        if (filePath == null || filePath.trim().isEmpty()) {
            Program current = programProvider.getCurrentProgram();
            if (current == null || current.getDomainFile() == null) {
                return "{\"error\": \"'path' parameter required when no current program is open\"}";
            }
            filePath = current.getDomainFile().getPathname();
        }
        ...
        status.put("lifecycle_scope", "project_domain_file");
        status.putAll(ProjectFileLifecycle.inspect(file));
